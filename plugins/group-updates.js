  ...
  async (message, match) => {
    if (!match[1] || !message.reply_message || !message.reply_message.sticker)
      return await message.sendReply(
        "_Sticker pe reply karo_\n_Ex: *.stickcmd kick*_"
      );
    try {
      await stickcmd.set(match[1], await extractData(message));
    } catch {
      return await message.sendReply("_Nakaam!_");
    }
    await message.client.sendMessage(
      message.jid,
      {
        text: `_Command chipka diya ${match[1]} to this sticker! Dobara connect ho raha hai..._`,
      },
      {
    ...
    if (message.reply_message && message.reply_message.sticker) {
      ...
      if (deleted)
        return await message.client.sendMessage(
          message.jid,
          {
            text: `_Sticker commands se hata diya!_`,
          },
          {
            quoted: message.quoted,
          }
      ...
      if (!deleted && match[1]) {
        ...
        if (delete_again)
          return await message.sendReply(
            `_Removed ${match[1]} chipkaye huay commands se!_`
          );
        if (!delete_again)
          return await message.sendReply("_Aisi koi sticker/command nahi mili!_");
      }
      if (!deleted && !match[1])
        return await message.send("_Aisi koi sticker nahi mili!_");
    } else if (match[1] && !message.reply_message) {
      let deleted = await stickcmd.delete(match[1], "command");
      if (deleted)
        return await message.sendReply(
          `_Kaamyabi se hata diya ${match[1]} chipkaye huay commands se!_`
        );
      if (!deleted)
        return await message.sendReply("_Aisi koi command nahi mili!_");
    } else
      return await message.sendReply(
        "_Command ya sticker pe reply karo!_\n_Ex: *.unstick kick*_"
      );
  }
);
...
Module(
  ...
  async (message, match) => {
    ...
    message.sendReply("_*Chipkaye huay commands:*_\n\n_" + msg + "_");
  }
);
