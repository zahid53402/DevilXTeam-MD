    ...
    if (message.fromOwner || adminAccesValidated) {
      ...
      if (match[1] === "on") {
        if (!(await isAdmin(message)))
          return await message.sendReply("_Main admin nahi hoon!_");
...
Module(
  ...
  async (message, match) => {
    ...
    if (message.fromOwner || adminAccesValidated) {
      ...
      await message.sendReply(
        match[1] === "on" ? "_Antispam chalu ho gaya!_" : "_Antispam band ho gaya!_"
      );
    }
  }
);
  ...
  async (message, match) => {
    ...
    if (message.fromOwner || adminAccesValidated) {
      ...
      if (match[1] !== "on" && match[1] !== "off") {
        ...
        var { subject } = await message.client.groupMetadata(message.jid);
        return await message.sendReply(
          `_Promote|demote alert menu of ${subject}_` +
            "\n\n_PDM alert abhi *" +
            status +
            "*_\n\n_Use .pdm on/off_"
        );
      ...
      await message.sendReply(
        match[1] === "on" ? "_Pdm chalu ho gaya!_" : "_Pdm band ho gaya!_"
      );
    }
  }
);
...
Module(
  ...
  async (message, match) => {
    ...
    await message.sendReply(
      match[1] === "on" ? "_Antidemote chalu ho gaya!_" : "_Antidemote band ho gaya!_"
    );
  }
);
...
Module(
  ...
  async (message, match) => {
    ...
    await message.sendReply(
      match[1] === "on"
        ? "_Antipromote chalu ho gaya!_"
        : "_Antipromote band ho gaya!_"
    );
  }
);
