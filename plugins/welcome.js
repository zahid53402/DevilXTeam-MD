...
Module(
  {
    pattern: "welcome ?(.*)",
    fromMe: false,
    desc: "Welcome message set karo for group. Use placeholders: $mention, $user, $group, $desc, $count, $pp, $gpp, $date, $time",
    usage:
  ...
  async (message, match) => {
    ...
    if (!input) {
      return await message.sendReply(`*Welcome Message ka Setup*

*Usage:*
• \`.welcome <message>\` - Welcome message set karo
• \`.welcome on/off\` - Welcome on/off karo
• \`.welcome get\` - Mojuda message dekho
• \`.welcome del\` - Welcome message delete karo
• \`.welcome status\` - Sab groups ka status dikaho (owner only)
• \`.welcome help\` - Tafseeli madad aur misalein
...
\`.welcome Welcome $user to our amazing group! We now have $count members! $gpp\``);
    ...
    if (input === "on") {
      ...
      if (!current) {
        return await message.sendReply(
          "_Koi welcome message set nahi! Pehle set karo:_\n*.welcome <your message>*"
        );
      }
      await welcome.toggle(message.jid, true);
      return await message.sendReply("_Welcome messages chalu ho gaye!_ ✅");
    }
    if (input === "off") {
      await welcome.toggle(message.jid, false);
      return await message.sendReply("_Welcome messages band ho gaye!_ ❌");
    }
    if (input === "get") {
      const current = await welcome.get(message.jid);
      if (!current) {
        return await message.sendReply(
          "_Is group ke liye koi welcome message nahi!_"
        );
      }
      return await message.sendReply(
        `*Mojuda Welcome Message:*\n\n${current.message}\n\n*Status:* ${
          current.enabled ? "Chalu ✅" : "Band ❌"
        }`
      );
    }
    if (input === "del" || input === "delete") {
      ...
      if (deleted) {
        return await message.sendReply(
          "_Welcome message delete ho gaya!_ 🗑️"
        );
      }
      return await message.sendReply("_Koi welcome message nahi mila delete karne ko!_");
    }
        ...
        if (welcomeData.length > 0) {
          ...
          for (let i = 0; i < welcomeData.length; i++) {
            ...
            try {
              ...
              const status = data.enabled ? "✅ Chalu" : "❌ Band";
              statusText += `${
                i + 1
...
• \`.goodbye <message>\` - Set goodbye message
• \`.goodbye on/off\` - Enable/disable goodbye
• \`.goodbye get\` - Mojuda message dekho
• \`.goodbye del\` - Delete goodbye message
*Placeholders:*
• \`$mention\` - Mention the user
• \`$user\` - User's name  
