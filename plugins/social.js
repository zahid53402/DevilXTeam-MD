...
Module(
  {
    pattern: "insta ?(.*)",
    fromMe: isFromMe,
    desc: "Instagram post/reel/tv download karta hai - supports multiple links",
  ...
  async (message, match) => {
    ...
    if (!instagramUrls.length)
      return await message.sendReply("_Sahi Instagram link do_");
    ...
    try {
      ...
      if (!allMediaUrls.length)
        return await message.sendReply(
          "_Kuch galat ho gaya, dobara try karo!_"
        );
      ...
      albumObject[0].caption = `_Download ho gaya! (${allMediaUrls.length} items)_`;
...
Module(
  {
    pattern: "fb ?(.*)",
    fromMe: isFromMe,
    desc: "Facebook video download karta hai",
...
Module(
  {
    pattern: "tiktok ?(.*)",
    fromMe: isFromMe,
    desc: "TikTok video download karta hai",
    usage: ".tiktok reply or link",
    use: "download",
  },
  async (message, match) => {
    let videoLink = match[1] !== "" ? match[1] : message.reply_message.text;
    if (!videoLink) return await message.sendReply("_TikTok URL chahiye_");
    videoLink = videoLink.match(/\bhttps?:\/\/\S+/gi)[0];
    let downloadResult;
    try {
