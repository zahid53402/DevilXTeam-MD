...
Module(
  {
    pattern: "filter ?(.*)",
    fromMe: false,
    desc: "Auto-reply filters banao. Usage: .filter trigger | response | scope(optional) | options(optional)",
    usage:
  ...
  async (message, match) => {
    ...
    if (!input) {
      return await message.sendReply(
        `*📝 Filter Commands:*\n\n` +
          ...
          `• \`${handler}filters\` - Sab filters dikhao\n` +
          `• \`${handler}delfilter trigger\` - Filter hatao\n` +
    ...
    if (!trigger || !response) {
      return await message.sendReply(
        "_Trigger aur jawab dono zaruri hain!_"
      );
    }
    ...
    try {
      ...
      const scopeText =
        scope === "chat"
          ...
          : `all ${scope}s`;
      const optionsText = [];
      if (filterOptions.exactMatch) optionsText.push("bilkul match");
      if (filterOptions.caseSensitive) optionsText.push("huroof ka farq");
      ...
      await message.sendReply(
        `✅ *Filter Ban Gaya!*\n\n` +
          `*Trigger:* ${trigger}\n` +
          `*Jawab:* ${response}\n` +
          `*Daira:* ${scopeText}${optionsStr}`
      );
    } catch (error) {
      console.error("Filter creation error:", error);
      await message.sendReply("_Filter banane mein nakaam!_");
    }
  }
);
  ...
  async (message, match) => {
    ...
    try {
      ...
      filters.forEach((f, index) => {
        ...
        msg += `   _Daira: ${f.scope}${f.enabled ? "" : " (disabled)"}_\n\n`;
      });
      ...
      if (matchedFilter) {
        await message.sendReply(
          `✅ *Filter Match Found!*\n\n` +
            ...
            `*Jawab:* ${matchedFilter.response}\n` +
            `*Daira:* ${matchedFilter.scope}\n` +
...
Module(
  {
    pattern: "filterhelp",
    fromMe: false,
    desc: "Filter system ki tafseeli madad",
    use: "utility",
  },
  async (message) => {
    const helpText =
      ...
      `• Use bilkul match for precise triggers\n` +
