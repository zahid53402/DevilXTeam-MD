...
async function parseAliveMessage(template, messageObject) {
  ...
  try {
    ...
    if (!parsedMessage.includes(config.CHANNEL_LINK)) {
      parsedMessage += `\n\n> 📢 _Join:_ ${config.CHANNEL_LINK}`;
    }
 ...
 */
async function getChannelContext(sock) {
  try {
...
async function sendAliveMessage(messageObject, parsedMessage) {
  ...
  try {
    const chCtx = await getChannelContext(messageObject.client);
