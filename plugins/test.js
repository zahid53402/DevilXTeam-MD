...
Module(
  {
    pattern: "age ?(.*)",
    desc: "Umar calculator .age dob",
    use: "utility",
  },
  async (m, t) => {
    if (!t[1]) return await m.sendReply("_Apni tareekh-e-paidaish do_");
    if (
      !/^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/.test(t[1])
    )
      return await m.sendReply("_Tareekh dd/mm/yyyy format mein honi chahiye_");
    var DOB = t[1];
...
Module(
  {
    pattern: "cntd ?(.*)",
    desc: "Tareekh ginta hai",
    use: "utility",
  },
  async (m, t) => {
    if (!t[1]) return await m.sendReply("_Aane wali tareekh do!_");
    if (
      !/^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/.test(t[1])
    )
      return await m.sendReply("_Tareekh dd/mm/yyyy format mein honi chahiye_");
    ...
    return await m.sendReply("_" + TimeCalculator(age) + " baqi_");
  }
);
Module(
  {
