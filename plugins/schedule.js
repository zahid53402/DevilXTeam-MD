...
Module(
  {
    pattern: "schedule ?(.*)",
    use: "utility",
    desc: "Baad mein bhejna ke liye message schedule karo. Reply to a message with: schedule <jid> <time> or schedule <time> <jid>",
  },
  async (m, match) => {
    if (match[1] === "d") return;
    if (!m.reply_message) {
...
Module(
  ...
  async (m, match) => {
    try {
      ...
      await m.sendReply("_Scheduled messages laane mein nakaam_");
    }
  }
);
Module(
  ...
  async (m, match) => {
    if (!match[1]) {
      return await m.sendReply(
        "_Cancel karne ke liye message ID do_\n\n*Usage:* cancel <id>"
      );
    }
    const messageId = parseInt(match[1].trim());
    if (isNaN(messageId)) {
      return await m.sendReply("_Sahi message ID do_");
    }
    try {
      const success = await scheduledMessages.delete(messageId);
      if (success) {
        await m.sendReply(
          `✅ *Scheduled message ${messageId} kaamyabi se cancel ho gaya*`
        );
      } else {
        await m.sendReply("❌ *Message nahi mila ya pehle bhej diya gaya*");
      }
    } catch (error) {
      console.error("Cancel scheduled error:", error);
      await m.sendReply("_Scheduled message cancel karne mein nakaam_");
    }
  }
);
