...
async function getRemoteVersion() {
  try {
    const remotePackageJsonUrl = `https://raw.githubusercontent.com/Devilkumail-MD/D.Kumail-MD/main/package.json`;
...
Module(
  ...
  async (message, match) => {
    ...
    const processingMsg = await message.sendReply("_Updates check ho rahe hain..._");
    ...
    try {
      ...
      if (!command) {
        ...
        if (isStableUpdate) {
          updateInfo = `*_UPDATE DASTIYAB_*\n\n`;
          updateInfo += `📦 Mojuda version: *${localVersion}*\n`;
          updateInfo += `📦 Nayi version: *${remoteVersion}*\n\n`;
          ...
          updateInfo = `*_BETA UPDATE DASTIYAB_*\n\n`;
          updateInfo += `📦 Mojuda version: *${localVersion}*\n`;
          updateInfo += `⚠️ Naye commits hain (version same hai)\n\n`;
          updateInfo += `*_CHANGELOG:_*\n\n`;
          for (let i in commits.all) {
            updateInfo += `${parseInt(i) + 1}• *${commits.all[i].message}*\n`;
          }
      ...
      return await message.edit(
        "_Updates check karte waqt galti ho gayi._",
        message.jid,
        processingMsg.key
      );
    }
