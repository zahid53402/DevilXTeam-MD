...
Module(
  {
    pattern: "warn ?(.*)",
    fromMe: false,
    desc: "Group mein user ko warning deta hai. Had puri hone pe nikal deta hai.",
  ...
  async (message, match) => {
    ...
    if (!message.isGroup)
      return await message.sendReply("_Ye sirf group command hai!_");
    ...
    if (!botIsAdmin) {
      return await message.sendReply(
        "_Mujhe warnings manage karne ke liye admin chahiye!_"
      );
    }
    ...
    if (!targetUser) {
      return await message.sendReply(
        `_User tag karo ya unke message pe reply karo!_\n\n` +
          `*Usage:*\n` +
          `• \`${handler}warn @user reason\`\n` +
          `• \`${handler}warn reply reason\`\n` +
          `• \`${handler}warnings @user\` - Warnings dekho\n` +
          `• \`${handler}rmwarn @user\` - Ek warning hatao\n` +
    ...
    if (isTargetAdmin) {
      return await message.sendReply("_Group admins ko warn nahi kar sakte!_");
    }

    const targetNumericId = targetUser?.split("@")[0];
    if (sudoUsers.includes(targetNumericId)) {
      return await message.sendReply("_Bot malik/sudo users ko warn nahi kar sakte!_");
    }

    let rawReason = match[1] || "Koi wajah nahi di gayi";
    const mentionRegex = new RegExp(`@${targetNumericId}\\s*`, "g");
    const reason =
      rawReason.replace(mentionRegex, "").trim() || "Koi wajah nahi di gayi";
        ...
        try {
          ...
          await message.client.sendMessage(message.jid, {
            text:
              ...
              `- Action: \`Group se nikal diya\`\n\n` +
              `_Warning had puri hone ki wajah se nikal diya gaya._`,
            mentions: [targetUser],
          });
        } catch (kickError) {
          await message.client.sendMessage(message.jid, {
            text:
              ...
              `- Error: \`User nikalne mein nakaam\`\n\n` +
              `_User khud nikalo ya meri admin permissions check karo._`,
            mentions: [targetUser],
          });
        }
      } else {
...
Module(
  {
    pattern: "warnings ?(.*)",
    fromMe: false,
    desc: "Warnings dekho for a user",
    usage: ".warnings @user\n.warnings reply",
    use: "group",
  },
  async (message, match) => {
...
Module(
  {
    pattern: "rmwarn ?(.*)",
    fromMe: false,
    desc: "Ek warning hatao for a user",
    usage: ".rmwarn @user\n.rmwarn reply",
    use: "group",
  },
  async (message, match) => {
